 ...
 * @since 9/8/2016
 */
@PipeBitInfo(name = "HTML Writer", description = "Writes html files with document text and simple markups (Semantic Group, CUI, Negation).", role = PipeBitInfo.Role.WRITER, dependencies = {
...
// TODO https://www.quackit.com/css/flexbox/tutorial/nested_flex_containers.cfm
	 ...
	 */
	@Override
	public void writeFile(final JCas jCas, final String outputDir, final String documentId, final String fileName)
			throws IOException {
		...
		try (final BufferedWriter writer = new BufferedWriter(new FileWriter(htmlFile))) {
			...
			final Map<Segment, Collection<org.apache.ctakes.typesystem.type.textspan.List>> lists = JCasUtil
					.indexCovered(jCas, Segment.class, org.apache.ctakes.typesystem.type.textspan.List.class);
			final Map<org.apache.ctakes.typesystem.type.textspan.List, Collection<ListEntry>> listEntries = JCasUtil
					.indexCovered(jCas, org.apache.ctakes.typesystem.type.textspan.List.class, ListEntry.class);
			...
			// TODO at each paragraph end index add a newline unless it is the end of a
			// section
			...
			writeSections(sections, paragraphs, lists, listEntries, sectionSentences, sentenceAnnotations,
					sentenceAnnotationsTime, sentenceTokens, relations, corefSpans, writer);
	 ...
	 */
	static private Map<TextSpan, Collection<Integer>> createCorefSpans(
			final Collection<CollectionTextRelation> corefRelations, final Map<Markable, TextSpan> markableSpans) {
	...
	static private Map<Markable, TextSpan> mapMarkableSpans(final JCas jCas,
		...
		final Map<ConllDependencyNode, Collection<IdentifiedAnnotation>> nodeAnnotations = JCasUtil.indexCovering(jCas,
				ConllDependencyNode.class, IdentifiedAnnotation.class);
	 ...
	 */
	static private Map<IdentifiedAnnotation, IdentifiedAnnotation> getAnnotationEvents(
...
//		+ getLegend() + "</nav>\n";
	 ...
	 * @param writer              writer to which pretty html for the section should
	 *                            be written
	...
	static private void writeSections(final Map<Segment, Collection<Sentence>> sectionSentences,
			...
			final Map<Sentence, Collection<Timex3>> sentenceAnnotationsTime,
			final Map<Sentence, Collection<BaseToken>> sentenceTokens, final Collection<BinaryTextRelation> relations,
			final Map<TextSpan, Collection<Integer>> corefSpans, final BufferedWriter writer) throws IOException {
		final Collection<Integer> paragraphBegins = paragraphs.stream().map(Annotation::getBegin)
		...
		for (Segment section : sections) {
			...
			for (Sentence sentence : sentences) {
				...
				writeSentence(sentence, annotations, annotationsTime, tokens, relations, corefSpans, writer);
	 ...
	 * @throws IOException if the writer has issues
	 */
	static private void writeSections(final Collection<Segment> sectionSet, final Collection<Paragraph> paragraphs,
			...
			final Map<TextSpan, Collection<Integer>> corefSpans, final BufferedWriter writer) throws IOException {
		if (lists.isEmpty()) {
			writeSections(sectionSentences, paragraphs, sentenceAnnotations, sentenceAnnotationsTime, sentenceTokens,
		...
		for (Map.Entry<org.apache.ctakes.typesystem.type.textspan.List, Collection<ListEntry>> entry : listEntries
				.entrySet()) {
		...
		for (Segment section : sections) {
			...
			for (Sentence sentence : sentences) {
				...
				if (end != null) {
					freshEntry = true;
					if (currentEnd < 0) {
						startList(sentence, annotations, annotationsTime, tokens, relations, corefSpans, writer);
						currentEnd = end;
					} else {
						writeListEntry(sentence, annotations, annotationsTime, tokens, relations, corefSpans, writer);
					}
				} else {
					if (currentEnd >= 0 && sentence.getBegin() > currentEnd) {
						endList(sentence, annotations, annotationsTime, tokens, relations, corefSpans, writer);
						currentEnd = -1;
						freshEntry = false;
						continue;
					}
	 ...
	 */
	static private String createLineText(final Sentence sentence, final Collection<IdentifiedAnnotation> annotations,
			final Collection<Timex3> annotationsTime, final Map<TextSpan, String> baseTokenMap,
		...
		final Map<TextSpan, Collection<Timex3>> annotationMapTime = createAnnotationMapTime(sentence, annotationsTime);
		...
		if (!annotationMapTime.isEmpty() && !annotationMap.isEmpty()) {
			tag = createTags(sentence, annotationMap, annotationEvents, relations, corefSpans);
			tag.putAll(createTagsTime(sentence, annotationMapTime, annotationEventsTime));

		} else if (!annotationMap.isEmpty()) {
			tag = createTags(sentence, annotationMap, annotationEvents, relations, corefSpans);
		} else if (!annotationMapTime.isEmpty()) {
			tag = createTagsTime(sentence, annotationMapTime, annotationEventsTime);
		}
		...
		for (Map.Entry<TextSpan, String> entry : baseTokenMap.entrySet()) {
			...
			if ((previousIndex!=-1) && (begin > previousIndex)){
				for(int i=0; i<begin-previousIndex; i++)
					sb.append(" ");
			}
			previousIndex = end;
			String temSB = "";
//			if (begin != previousIndex) {
				...
				for (String temChar : text.split("")) {
					if (isBegin) {
						...
						if (beginTag != null && !beginTag.equals("</span>")) {
							sb.append(temSB);
							temSB = "";
							sb.append(beginTag);
							isBegin = false;
...
//			previousIndex = end;
	...
	static private void startList(final Sentence sentence, final Collection<IdentifiedAnnotation> annotations,
		...
		writer.write("\n<ul>\n<li>");
		final String lineText = createLineText(sentence, annotations, annotationsTime, baseTokenMap, relations,
				corefSpans);
	 ...
	 * @param relations   all relations
	 * @param writer      writer to which pretty html for the section should be
	 *                    written
	 * @throws IOException if the writer has issues
	 */
	static private void writeListEntry(final Sentence sentence, final Collection<IdentifiedAnnotation> annotations,
			final Collection<Timex3> annotationsTime, final Collection<BaseToken> baseTokens,
			final Collection<BinaryTextRelation> relations, final Map<TextSpan, Collection<Integer>> corefSpans,
			final BufferedWriter writer) throws IOException {
		if (baseTokens.isEmpty()) {
	...
	static private void endList(final Sentence sentence, final Collection<IdentifiedAnnotation> annotations,
			final Collection<Timex3> annotationsTime, final Collection<BaseToken> baseTokens,
			final Collection<BinaryTextRelation> relations, final Map<TextSpan, Collection<Integer>> corefSpans,
			final BufferedWriter writer) throws IOException {
		if (baseTokens.isEmpty()) {
	...
	/**
	 * removes empty spans and replaces non-html compatible characters with their
	 * html ok equivalents
	 *
	 * @param sentence   -
...
//			String text = getSafeText(baseToken);
	...
	static private Map<TextSpan, Collection<Timex3>> createAnnotationMapTime(final Sentence sentence,
	 ...
	 * @param annotationMap map of all annotations within or overlapping the small
	 *                      span elements
	 * @param relations     all relations
	 * @param corefSpans    map of text span ends to coreference chain indices
...
////					if (semanticCode.equals(EVENT_CODE) || semanticCode.equals(TIMEX_CODE)
////							|| semanticCode.equals(ENTITY_CODE) || semanticCode.equals(UNKNOWN_SEMANTIC_CODE)) {
////						continue;
////					}
...
////			}
	...
	static private Map<Integer, String> createTagsTime(final Sentence sentence,
		...
		for (TextSpan adjustedSpan : adjustedList) {
			...
			final String polarityClasses = createPolaritiesTextTime(annotations);
			if (!polarityClasses.isEmpty()) {
				sb.append(" class=\"").append(polarityClasses).append('\"');
			}
			...
			final String tip = createTipTextTime(annotations);
			if (!tip.isEmpty()) {
				sb.append(" " + TOOL_TIP + "=\"").append(tip).append('\"');
			}
			sb.append('>');
			...
			final int adjustedEnd = sentenceBegin + adjustedSpan.getEnd();
			final StringBuilder sb2 = new StringBuilder();
			final Collection<Timex3> endAnnotations = getEndAnnotationsTime(annotations, adjustedEnd);
//			 final Collection<String> semanticCodes =
//			 SemanticGroup.getSemanticCodes(endAnnotations);
//			String semantic = "semantiCodes";// semanticCodes.stream().findAny().orElse(ENTITY_CODE);
//			if (annotations != null && endAnnotations.size() != annotations.size()) {
//				semantic += " " + polarityClasses;
//			}
//			final Collection<Integer> chains = sentenceCorefs.get(adjustedSpan);
...
//			}
	...
	static private Collection<IdentifiedAnnotation> getEndAnnotations(
			final Collection<IdentifiedAnnotation> annotations, final int adjustedEnd) {
		if (annotations == null || annotations.isEmpty()) {
			return Collections.emptyList();
		}
	...
	static private Collection<Timex3> getEndAnnotationsTime(final Collection<Timex3> annotations,
	...
	static private String createClickInfoTime(final Collection<Timex3> annotations,
		...
		for (Timex3 annotation : annotations) {
			...
			polarInfoMap.putIfAbsent(polarity, new HashMap<>());
			final Timex3 event = annotationEvents.get(annotation);
	...
	static private Map<String, Collection<String>> createInfoMap(final IdentifiedAnnotation annotation,
		...
		for (UmlsConcept concept : concepts) {
			final String semanticCode = "TUI: " + SemanticGroup.getSemanticWord(concept);
			semanticMap.putIfAbsent(semanticCode, new HashSet<>());
			final String prefText = getPreferredText(coveredText, concept);
			String text = "LINK: " + getWikiText(safeText, prefText) + NEWLINE + getCodes(concept)
					+ getCodedPrefText(prefText) + relationText;
	...
	static private Map<String, Collection<String>> createInfoMapTime(final Timex3 annotation) {
		...
		final String safeText = "TEXT: " + getSafeText(coveredText);
		String relationText = annotation.getTimexMod();
		String type = "TYPE: " + annotation.getTimexType();
		String value = "VALUE: " + annotation.getTimexValue();
		if (!type.isEmpty()) {
			semanticMap.putIfAbsent(type, new HashSet<>());
			semanticMap.get(type).add(safeText + NEWLINE + value + NEWLINE);
		}
...
//	static private Map<String, Collection<String>> createInfoMapTime(final Timex3 annotation,
//			final Timex3 event, final Collection<BinaryTextRelation> relations) {
//		final Collection<UmlsConcept> concepts = OntologyConceptUtil.getUmlsConcepts(annotation);
//		final Map<String, Collection<String>> semanticMap = new HashMap<>();
//		final String coveredText = getCoveredText(annotation);
//		final String safeText = getSafeText(coveredText);
...
//			semanticMap.putIfAbsent(semanticCode, new HashSet<>());
//			final String prefText = getPreferredText(coveredText, concept);
//			String text = getWikiText(safeText, prefText) + NEWLINE + getCodes(concept) + getCodedPrefText(prefText)
//					+ relationText;
//			if (annotation instanceof EventMention) {
//				text += getDocTimeRel((EventMention) annotation);
//			}
...
//	}
	...
	static private String getCodes(final UmlsConcept concept) {
		...
		if (cui != null && !cui.isEmpty()) {
			codes += "CUI: " + cui + NEWLINE;
		}
		final String code = concept.getCode();
		if (code != null && !code.isEmpty()) {
			codes += "CUI: " + code + NEWLINE;
		}
		return codes;
	}
	...
	static private String getPreferredText(final String coveredText, final UmlsConcept concept) {
		...
		if (preferredText != null && !preferredText.isEmpty() && !preferredText.equals(PREFERRED_TERM_UNKNOWN)
				&& !preferredText.equalsIgnoreCase(coveredText) && !preferredText.equalsIgnoreCase(coveredText + 's')
	...
	static private String getWikiText(final String coveredText, final String preferredText) {
		...
		// oddly enough, searches more frequently have the covered text instead of the
		// preferred text
...
//      return WIKI_BEGIN + wikiText.replace( ' ', '%' ).toLowerCase() + WIKI_CENTER + coveredText + WIKI_END;  // webmd
		return WIKI_BEGIN + wikiText.replace(' ', '+').toLowerCase() + WIKI_CENTER + coveredText + WIKI_END; // most
																												// sites
	}
	...
	static private String createPolaritiesText(final Collection<IdentifiedAnnotation> annotations) {
		...
		return annotations.stream().map(HtmlTextWriter::createPolarity).distinct().sorted()
				.collect(Collectors.joining(" "));
	}

	static private String createPolaritiesTextTime(final Collection<Timex3> annotations) {
		return GENERIC;
	}
	...
	static private String createTipTextTime(final Collection<Timex3> annotations) {
		if (annotations == null || annotations.isEmpty()) {
			return "";
		}
		final Map<String, Integer> semanticCounts = getSemanticCountsTime(annotations);
		final List<String> semantics = new ArrayList<>(semanticCounts.keySet());
		Collections.sort(semantics);
		final StringBuilder sb = new StringBuilder();
		for (String semanticName : semantics) {
	...
	static private Map<String, Integer> getSemanticCounts(final Collection<IdentifiedAnnotation> annotations) {
		// Check concepts with the same cui can have multiple tuis. This can make it
		// look like there are extra counts.
	...
	static private Map<String, Integer> getSemanticCountsTime(final Collection<Timex3> annotations) {
		// Check concepts with the same cui can have multiple tuis. This can make it
		// look like there are extra counts.
		final Collection<String> usedCuis = new HashSet<>();
		final Map<String, Integer> semanticCounts = new HashMap<>();
...
//			}
	...
	/**
	 * This method needs to be in this class so that it can properly link the coref
	 * chain numbers
	 * 
	 * @param corefRelations -
	 * @param writer         writer to which pretty html for the section should be
	 ...
	 * @throws IOException if the writer has issues
	 */
	static private void writeCorefInfos(final Collection<CollectionTextRelation> corefRelations,
			final BufferedWriter writer) throws IOException {
		if (corefRelations == null || corefRelations.isEmpty()) {
			return;
		}
		...
		for (CollectionTextRelation corefRelation : corefRelations) {
			final FSList chainHead = corefRelation.getMembers();
			final Collection<IdentifiedAnnotation> markables = FSCollectionFactory.create(chainHead,
					IdentifiedAnnotation.class);
			final String text = markables.stream().sorted(Comparator.comparingInt(Annotation::getBegin))
					.map(HtmlTextWriter::getSafeText).collect(Collectors.joining("<br>"));
