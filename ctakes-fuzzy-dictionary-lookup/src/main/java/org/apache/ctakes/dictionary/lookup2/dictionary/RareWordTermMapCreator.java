 ...
 * {@link org.apache.ctakes.dictionary.lookup2.dictionary.RareWordTermMapCreator.CuiTerm}
 * Objects, this factory can create a Map of
 * {@link org.apache.ctakes.dictionary.lookup2.term.RareWordTerm} collections
 * indexed by rare word. This map can be used to create a
 * {@link MemRareWordDictionary}
 * <p/>
...
final public class RareWordTermMapCreator {
	...
	static private final Collection<String> PREFIXES = new HashSet<>(Arrays.asList("e-", "a-", "u-", "x-", "agro-",
			...
			"ex-", "extra-", "inter-", "intra-", "macro-", "mega-", "micro-", "mid-", "mini-", "multi-", "neo-", "non-",
			"over-", "pan-", "para-", "peri-", "post-", "pre-", "pro-", "pseudo-", "quasi-", "re-", "semi-", "sub-",
			"super-", "tri-", "ultra-", "un-", "uni-", "vice-",
...
//                                              "-o-torium", "-rama", "-wise" };
	...
	static private final Collection<String> SUFFIXES = new HashSet<>(Arrays.asList("-esque", "-ette", "-fest", "-fold",
			"-gate", "-itis", "-less", "-most", "-o-torium", "-rama", "-wise"));
	...
	// Listing every verb in the language seems a pain, but listing the others is
	// possible.
	// Verbs should be rare in the dictionaries, excepting perhaps the activity and
	...
	static private final Collection<String> BAD_POS_TERMS = new HashSet<>(Arrays.asList(
			...
			"about", "off", "up", "along", "away", "back", "by", "down", "forward", "in", "on", "out", "over", "around",
			"under",
...
//	}
	...
	static private List<String> getTokens(final String word) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ((c == '\'' && isOwnerApostrophe(word, i + 1)) || (c == '.' && isNumberDecimal(word, i + 1))) {
			...
			// Wasn't a special symbol for consideration, so add the previous and symbol
			// separately
			if (sb.length() != 0) {
				tokens.add(sb.toString());
				sb.setLength(0);
...
//      return sb.toString();
	...
	static public class CuiTerm {
		...
		public boolean equals(final Object value) {
			return value instanceof CuiTerm && __term.equals(((CuiTerm) value).__term)
