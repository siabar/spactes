...
/**
 * A RareWordDictionary created from a bar-separated value (BSV) file. The file
 * can have 2 or 3 columns, in the format CUI|TEXT or CUI|TUI|TEXT. The text
 * will be tokenized and rare word indexing done automatically for internal
 * storage and retrieval. If TUI is not supplied then CUI duplicates as TUI.
 * This dictionary is really just a wrapper of a {@link MemRareWordDictionary}
 * with a file reader. Author: SPF Affiliation: CHIP-NLP Date: 1/9/14
 */
final public class BsvRareWordDictionary implements RareWordDictionary {
	...
	public BsvRareWordDictionary(final String name, final String bsvFilePath) {
		...
		final CollectionMap<String, RareWordTerm, ? extends Collection<RareWordTerm>> rareWordTermMap = RareWordTermMapCreator
				.createRareWordTermMap(cuiTerms);
	 ...
	 * Objects by parsing a bsv file. The file can be in one of two columnar
	 * formats:
	 * <p>
	 * CUI|Text
	 * </p>
	 ...
	 * </p>
	 * If the TUI column is omitted then the entityId for the dictionary is used as
	 * the TUI
	 * <p/>
	 *
