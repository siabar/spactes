...
/**
 * Performs the basic initialization with uima context, including the parse of
 * the dictionary specifications file. Has a
 * <p/>
 * Author: SPF Affiliation: CHIP-NLP Date: 12/6/13
 */
abstract public class AbstractJCasTermAnnotator extends JCasAnnotator_ImplBase
	...
	@ConfigurationParameter(name = ConfigParameterConstants.PARAM_LOOKUP_XML, mandatory = false, description = ConfigParameterConstants.DESC_LOOKUP_XML, defaultValue = DEFAULT_DICT_DESC_PATH)
	...
	public final Boolean lemmaForm = false;

	/**
	 * @deprecated replaced by _lookupXml
	 */
	@Deprecated
	@ConfigurationParameter(name = JCasTermAnnotator.DICTIONARY_DESCRIPTOR_KEY, mandatory = false, description = "Path to Dictionary spec xml", defaultValue = DEFAULT_DICT_DESC_PATH)
	private String _descriptorFilePath;

	// type of lookup window to use, typically "LookupWindowAnnotation" or
	// "Sentence"
	@ConfigurationParameter(name = JCasTermAnnotator.PARAM_WINDOW_ANNOT_KEY, mandatory = false, description = "Type of Lookup window to use", defaultValue = DEFAULT_LOOKUP_WINDOW)
	private String _windowClassName;

	// set of exclusion POS tags (lower cased), may be null
	@ConfigurationParameter(name = JCasTermAnnotator.PARAM_EXC_TAGS_KEY, mandatory = false, description = "Set of exclusion POS tags", defaultValue = DEFAULT_EXCLUSION_TAGS)
...
//         return;
//      }
		final Map<Annotation, Collection<BaseToken>> windowTokens = org.apache.uima.fit.util.JCasUtil.indexCovered(jcas,
				_lookupClass, BaseToken.class);
		final Map<RareWordDictionary, CollectionMap<TextSpan, Long, ? extends Collection<Long>>> dictionaryTermsMap = new HashMap<>(
...
//            }
			...
			// JCasHashMap will throw this every once in a while. Assume the windows are
			// done and move on
		...
		for (Map.Entry<RareWordDictionary, CollectionMap<TextSpan, Long, ? extends Collection<Long>>> dictionaryCuis : dictionaryTermsMap
			...
			final Collection<ConceptFactory> conceptFactories = _dictionarySpec
					.getPairedConceptFactories(dictionary.getName());
...
//   }
	...
	/**
	 * Given a set of dictionaries, tokens, and lookup token indices, populate a
	 * terms map with discovered terms
	 *
	 * @param dictionaries       -
	 * @param allTokens          -
	 ...
	 */
	private void findTerms(final Iterable<RareWordDictionary> dictionaries, final List<FastLookupToken> allTokens,
...
//   }
	...
	/**
	 * For the given lookup window fills two collections with 1) All tokens in the
	 * window, and 2) indexes of tokens in the window to be used for lookup
	 *
	 * @param jcas               -
	...
	protected void getAnnotationsInWindow(final JCas jcas, final Collection<BaseToken> windowBaseTokens,
			final List<FastLookupToken> allTokens, final Collection<Integer> lookupTokenIndices) {
//      final List<BaseToken> allBaseTokens = org.apache.uima.fit.util.JCasUtil
//            .selectCovered( jcas, BaseToken.class, window );
		int counter = 1;
		if (windowBaseTokens.size() >= 2) {
			BaseToken Token_x = null;
			for (BaseToken Token : windowBaseTokens) {
				Token_x = Token;
			}
			if (Token_x.getCoveredText().contentEquals("."))
				counter = windowBaseTokens.size() -1;
			else
				counter = windowBaseTokens.size();
		}
		for (BaseToken baseToken : windowBaseTokens) {
			if (baseToken instanceof NewlineToken) {
				continue;
			}
//			final boolean isNonLookup = baseToken instanceof PunctuationToken || baseToken instanceof NumberToken
//					|| baseToken instanceof ContractionToken || baseToken instanceof SymbolToken;
//			// We are only interested in tokens that are -words-
//			if (!isNonLookup) {
//				// POS exclusion logic for first word lookup
//				final String partOfSpeech = baseToken.getPartOfSpeech();
...
//			}
			...
			if (lemmaForm) {
				FSList fsList = baseToken.getLemmaEntries();

				for (TOP inte : fsList) {
					Lemma lemmaa = (Lemma) inte;
					temp = lemmaa.getKey();
				}

			}

			final FastLookupToken lookupToken = new FastLookupToken(baseToken, counter, temp);
			allTokens.add(lookupToken);
		}
	}
